...
function commitEffectHook() {
  function run(fiber) {
    if (!fiber) return;
    if (!fiber.alternate) {
      //fiber 没有alternate，说明是第一次渲染，执行初始化-effectHook
      ...
      fiber.effectHooks?.forEach((newHook, index) => {
        ...
        let needUpdate = oldHook.deps.some((oldDep, i) => {
  ...
  // clearup要在当前effect执行之前，执行
  // 也是之前所有的effect执行完之后，才执行
  // 所以这里采用的是fiber的alternate的effectHooks
...
function useEffect(callback, deps) {
  let effectHook = { callback, deps, cleanUp: undefined };
